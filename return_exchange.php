...
if (isset($_GET['sale_id'])) {
    ...
    $saleQuery = "SELECT s.sale_id, s.invoice_date, s.product_name, s.sale_quantity, s.sale_price, 
...
<body>
    ...
    <form id="purchaseForm" method="POST" action="">
        ...
        <div class="form-group">
            ...
            <input type="text" id="vehicleName" name="vehicleName" value="<?php echo htmlspecialchars($saleData['vehicle_name']); ?>" readonly>
        </div>
        <div class="form-group">
...
<table id="productTable">
   ...
   foreach ($productNames as $index => $productName) {
    ...
    $totalStock = isset($stockData[$productName]) ? $stockData[$productName] : 0;
        ...
        echo "<td id='price_$i'>" . htmlspecialchars($prices[$i]) . "</td>";
        echo "<td id='quantity_$i'>" . htmlspecialchars($quantities[$i]) . "</td>";
        echo "<td id='total_$i'>" . htmlspecialchars($prices[$i] * $quantities[$i]) . "</td>";
        echo "<td>
            <input type='number' id='action_quantity_$i' min='1' placeholder='Enter quantity'>
            <button type='button' onclick='confirmAction(\"" . htmlspecialchars($productNames[$i]) . "\", $i, \"decrement\")'>-</button>
            <button type='button' onclick='confirmAction(\"" . htmlspecialchars($productNames[$i]) . "\", $i, \"increment\")'>+</button>
            <button type='button' onclick='confirmDelete(\"" . htmlspecialchars($productNames[$i]) . "\", $i)'>Delete</button>
        </td>";
...
// Function to recalculate the net total including vehicle expense
// Function to recalculate the net total including vehicle expense and discount
// function recalculateNetTotal() {
//     let netTotal = 0;
//     const table = document.getElementById("productTable");

//     // Loop through all rows and calculate the net total from the product table
//     for (let i = 1; i < table.rows.length; i++) {
//         const rowTotal = parseFloat(document.getElementById("total_" + (i - 1)).innerText) || 0;
//         netTotal += rowTotal;
//     }

//     // Get the vehicle expense and add it to the net total
//     const vehicleExpense = parseFloat(document.getElementById("vehicleExpense").value) || 0;
//     netTotal += vehicleExpense;
...
//     // Recalculate grand total after discount
//     recalculateGrandTotal();
// }
