    ...
    if (!empty($productNames) && count($productNames) > 0 && !empty($productNames[0])) {
        try {
            ...
            foreach ($productNames as $index => $productName) {
                ...
                if ($productId === null) {
                    // Insert the new product into the `products` table and retrieve the product ID
                    $insertProductQuery = "INSERT INTO products (product_name) VALUES (?)";
                    $stmtInsertProduct = $conn->prepare($insertProductQuery);
                    $stmtInsertProduct->bind_param("s", $productName);
                    $stmtInsertProduct->execute();
                    $productId = $conn->insert_id; // Get the newly inserted product ID
                    $stmtInsertProduct->close();
                }

                // Step 2: Check stock and update or insert it
                $currentStock = null;
                $fetchStockQuery = "SELECT total_quantity FROM stock WHERE product_id = ?";
                $stmtFetchStock = $conn->prepare($fetchStockQuery);
                $stmtFetchStock->bind_param("i", $productId);
                $stmtFetchStock->execute();
                $stmtFetchStock->bind_result($currentStock);
                $stmtFetchStock->fetch();
                $stmtFetchStock->close();

                if ($currentStock === null) {
                    // Insert new stock if none exists
                    $insertStockQuery = "INSERT INTO stock (product_id, name, total_quantity, sold_quantity) VALUES (?, ?, ?, 0)";
                    $stmtInsertStock = $conn->prepare($insertStockQuery);
                    $stmtInsertStock->bind_param("isi", $productId, $productName, $quantity);
                    $stmtInsertStock->execute();
                    $stmtInsertStock->close();
                } else {
                    // Update existing stock
                    $newQuantity = $currentStock + $quantity;
                    $updateStockQuery = "UPDATE stock SET total_quantity = ? WHERE product_id = ?";
                    $stmtUpdateStock = $conn->prepare($updateStockQuery);
                    $stmtUpdateStock->bind_param("ii", $newQuantity, $productId);
                ...
                if ($purchaseResult) {
                    if ($purchaseResult->num_rows > 0) {
                        ...
                        while ($purchase = $purchaseResult->fetch_assoc()) {
                            ...
                            echo "<a href='Action/Print/generate_invoice2.php?purchase_id={$purchase['purchase_id']}' onclick='printInvoice(event)'>";
