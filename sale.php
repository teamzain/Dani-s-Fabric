...
if (isset($_POST['submit'])) {
    ...
    if (!empty($productNames) && count($productNames) > 0 && !empty($productNames[0])) {
        try {
            ...
            $sql = "INSERT INTO sale (invoice_date, customer_id, vehicle_name, net_total, grand_total, paid_amount, due_amount, payment_type, product_name, sale_quantity, sale_price, total_amount, vehicle_expense) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                if ($saleResult) {
                    if ($saleResult->num_rows > 0) {
                        ...
                        while ($sale = $saleResult->fetch_assoc()) {
                            ...
                            echo "<a href='#' class='edit-btn' data-sale-id='{$sale['sale_id']}' data-paid-amount='{$sale['paid_amount']}' data-due-amount='{$sale['due_amount']}' data-grand-total='{$sale['grand_total']}'>";
